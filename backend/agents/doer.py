...
def detect_action_intent(transcript: str) -> str | None:
    """If a journal entry clearly asks for something to be DONE (not just
    felt), return the sentence to turn into a prepared action. The author
    still approves before anything runs — this only drafts."""
    if not _DELEGATE_RE.search(transcript) and not re.search(
...
_ITEM_RE = re.compile(
    ...
    r"(?:\s+for\s+(?:my|her|his|our|the)\b.*)?$",
    re.I,
)
...
def _prepare_purchase(action: Action, plan: dict) -> dict:
    ...
    useful link, so approval always ends in something actionable."""
    item = plan.get("item", "")
    site = plan.get("site") or settings.doer_purchase_site
    search_url = (
        f"https://www.amazon.in/s?k={quote(item)}"
        if "amazon" in site
        else site
    )
    manual = {
    ...
    try:
        ...
        browser = pw.chromium.launch(headless=settings.doer_headless)
        page = browser.new_page(
            viewport={"width": 1280, "height": 860}, user_agent=BROWSER_UA
        )
        try:
            if "amazon" in site:
...
def _amazon_cart(page, item: str) -> None:
    ...
    href = first.get_attribute("href", timeout=10000)
    if not href:
        raise RuntimeError("no search results")
    _goto_with_retry(
        page, href if href.startswith("http") else f"https://www.amazon.in{href}"
    )
